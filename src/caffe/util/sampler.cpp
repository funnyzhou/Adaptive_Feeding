...
bool SatisfySampleConstraint(const NormalizedBBox& sampled_bbox,
  ...
  for (int i = 0; i < object_bboxes.size(); ++i) {
    ...
    if (((object_bbox.xmax() - object_bbox.xmin()) > sample_constraint.max_box_scale()) ||
	((object_bbox.ymax() - object_bbox.ymin()) > sample_constraint.max_box_scale())){
	continue;
    }	
